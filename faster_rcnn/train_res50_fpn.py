...
def create_model(num_classes, load_pretrain_weights=True):
    ...
    backbone = resnet50_fpn_backbone(pretrain_path="./backbone/resnet50.pth", norm_layer=torch.nn.BatchNorm2d,
                                     trainable_layers=3)

    # please notice that, `num_classes` in FasterRCNN represents `obj_classes + background`;
    # when training with pretrained model in COCO datasets, the following `num_classes` should be `91`.
    model = FasterRCNN(backbone=backbone, num_classes=91)

    # whether load pretrained model in COCO datasets, default == True
...
def main(args):
    ...
    VOC_root = os.path.join(args.data_path, '..', '..', 'CLOTH-OD')
    # check voc root
    ...
    if args.aspect_ratio_group_factor >= 0:
        train_sampler = torch.utils.data.RandomSampler(train_dataset)  # sampling datasets, without replacement
    ...
    learning_rate = []  # record learning rate
    val_map = []        # record pascal mAP

    # ---> training cycle <---
    for epoch in range(args.start_epoch, args.epochs):
        # train for one epoch, printing every 10 iterations
        mean_loss, lr = utils.train_one_epoch(model, optimizer, train_data_loader, device=device, epoch=epoch,
...
if __name__ == "__main__":
    ...
    # 检测目标类别数(不包含背景)
    parser.add_argument('--num-classes', default=33, type=int, help='num_classes')
    # 文件保存地址
