...
def batched_nms(boxes, scores, idxs, iou_threshold):
    ...
    # 为每一个类别(prediction)/每个预测特征层（training）生成一个很大的偏移量
    offsets = idxs.to(boxes) * (max_coordinate + 1)
    # boxes加上对应层的偏移量后，保证不同类别(prediction)/层(level)之间boxes不会有重合的现象
    # 这样设置偏移量的技巧，可以完成仅使用一次NMS就可以为每个类别/层过滤bbox
...
def clip_boxes_to_image(boxes, size):
    ...
    Note:
        In the code `boxes[..., 1::2]`, Ellipsis `...` means [:, 1::2], [:, :, 1::2], etc.
        while `a::b` means slicing start at `a` with interval `b`.
    """
    dim = boxes.dim()
