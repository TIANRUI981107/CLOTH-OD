...
class BoxCoder(object):
    ...
    def decode(self, rel_codes, boxes):
        ...
        """
        Apply `regressed offsets` to `anchors' coordinates`.

        Args:
            rel_codes: bbox regression `offset` parameters
            boxes: anchors/proposals `coordinates`
        ...
        concat_boxes = torch.cat(boxes, dim=0)  # concatenate anchors along all batch_size dimension

        # get the number of boxes given a batched tensor
    ...
    def decode_single(self, rel_codes, boxes):
        ...
        Note:
            ...
            here, t stands for pred_bboxes, a stands for anchors.

            torch.stack() vs torch.cat():
                1. torch.stack(seq, dim=k) create a new dimension at k dim, and stack the others along this dim;
                2. torch.cat does not create new dims
        """
        ...
        wx, wy, ww, wh = self.weights  # RPN中为[1,1,1,1], fast r-cnn中为[10,10,5,5]
        # Note that slicing method x[:, 0::4] keep the second dims untouched: [num_row] -> [num_row, 1]
        ...
        # apply regressed offsets to anchors for pred_boxes
        ...
        pred_boxes = torch.stack((pred_boxes1, pred_boxes2, pred_boxes3, pred_boxes4), dim=2).flatten(1)  # what's for?
        return pred_boxes
